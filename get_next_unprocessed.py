#!/usr/bin/env python3
"""Get next batch of truly unprocessed notes"""
import json
...
# Find unprocessed notes, starting with non-philosophy folders
unprocessed = []
philosophy_unprocessed = []
...
for note_id, metadata in vault_data["notes_metadata"].items():
    ...
    if path.startswith("800_Ressources/") and not metadata.get("ai_summary"):
        if "100_Philosophy" in path:
            philosophy_unprocessed.append((note_id, metadata))
        else:
            unprocessed.append((note_id, metadata))

# Sort by path
unprocessed.sort(key=lambda x: x[1].get("path", ""))
philosophy_unprocessed.sort(key=lambda x: x[1].get("path", ""))

# Combine - non-philosophy first
all_unprocessed = unprocessed + philosophy_unprocessed

print(f"Unprocessed notes:")
print(f"  - Non-philosophy: {len(unprocessed)}")
print(f"  - Philosophy: {len(philosophy_unprocessed)}")
print(f"  - Total: {len(all_unprocessed)}")

# Take next 10
batch = all_unprocessed[:10]
